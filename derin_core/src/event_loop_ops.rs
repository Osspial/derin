...
pub struct ActiveWidgets {
    mouse_l: Option<WidgetID>,
    mouse_r: Option<WidgetID>,
    mouse_m: Option<WidgetID>,
    mouse_x1: Option<WidgetID>,
    mouse_x2: Option<WidgetID>,
    mouse_hover: Option<WidgetID>,
    keyboard: Option<WidgetID>,
} // TODO: CONVERT move_over_flags TO ACTIVEWIDGETS WITH MOVING TO WIDGET ID

impl ActiveWidgets {
    fn widgets_pressed(&self) -> impl Iterator<Item=WidgetID> + Clone {
        self.mouse_l.into_iter()
            .chain(self.mouse_r)
            .chain(self.mouse_m)
            .chain(self.mouse_x1)
            .chain(self.mouse_x2)
          ...
          F: RenderFrame,
{
    ...
    fn process_event_inner(
        ...
        match event {
            ...
            WindowEvent::MouseEnter(enter_pos) => {
                ...
                let top_widget_tag = try_push_action!(
                ...
                active_widgets.mouse_hover = Some(top_widget_tag.widget_id());
            },
            WindowEvent::MouseExit(exit_pos) => {
                if let Some(mouse_hover) = active_widgets.mouse_hover {
                    if let Some(_) = widget_stack.move_to_widget(mouse_hover) {
                        widget_stack.drain_to_root(|mut widget, path| {
                            let widget_tag = try_push_action!(
            ...
            WindowEvent::MouseMove(new_pos) if new_pos != *mouse_pos => {
                ...
                *mouse_pos = new_pos;
                    ...
                    if let Some(mouse_hover) = active_widgets.mouse_hover {
                        ...
                        match top_bounds.map(|r| r.contains(new_pos)).unwrap_or(false) {
                            true => {
                                ...
                                let top_widget_id = top_widget.widget_tag().widget_id();

                                // Set the top widget's mouse state to hovering.
                                widget_state_map.entry(top_widget_id).or_default().mouse_state = MouseState::Hovering(new_pos, mbseq);
                                active_widgets.mouse_hover = Some(top_widget_id);
                                ...
                                if let Some(mut top_widget_as_parent) = top_widget.as_parent_mut() {
                                    ...
                                    top_widget_as_parent.children_mut(|mut child_summary| {
                                        ...
                                        let child_widget_id = child.widget_tag().widget_id();
                                        ...
                                        // state to hover.
                                        let mouse_state = &mut widget_state_map.entry(child_widget_id).or_default().mouse_state;
                                        let child_mbdin = mouse_state.mouse_button_sequence();
                                        *mouse_state = MouseState::Hovering(widget_old_pos, child_mbdin);
                                        active_widgets.mouse_hover = Some(child_widget_id);
                            ...
                            false => {
                                ...
                                if 0 < widget_stack.depth() {
                                    ...
                                    active_widgets.mouse_hover = Some(widget_state.widget_id());
                        ...
                        let root_widget_id = root.widget_tag().widget_id();

                        let top_widget_state = widget_state_map.entry(root_widget_id).or_default();
                ...
                widget_stack.move_over_widgets(active_widgets.widgets_pressed(), |mut widget, path, _| {
            ...
            WindowEvent::MouseDown(button) => {
                let move_over = active_widgets.mouse_hover.into_iter().chain(active_widgets.keyboard);
                widget_stack.move_over_widgets(move_over, |mut top_widget, path, _| {
                    let top_widget_state = try_push_action!(
                    ...
                    let active_button_field = match button {
                        MouseButton::Left => &mut active_widgets.mouse_l,
                        MouseButton::Right => &mut active_widgets.mouse_r,
                        MouseButton::Middle => &mut active_widgets.mouse_m,
                        MouseButton::X1 => &mut active_widgets.mouse_x1,
                        MouseButton::X2 => &mut active_widgets.mouse_x2,
                    };
                    *active_button_field = Some(top_widget_state.widget_id());
                });
            ...
            WindowEvent::MouseUp(button) => {
                ...
                *active_button_field = None;

                // Send the mouse up event to the hover widget.
                let mut move_to_tracked = true;
                widget_stack.move_over_widgets(active_widgets.mouse_hover, |mut widget, path, _| {
            ...
            WindowEvent::MouseScrollLines(scroll_lines) => {
                if let Some(WidgetPath{ widget: mut hover_widget, path }) = active_widgets.mouse_hover.and_then(|mh| widget_stack.move_to_widget(mh)) {
            ...
            WindowEvent::KeyDown(key) => {
                ...
                if let Some(WidgetPath{ widget: mut focus_widget, path }) = active_widgets.keyboard.and_then(|kbd| widget_stack.move_to_widget(kbd)) {
        ...
        loop {
            ...
            | {
                if let Some(WidgetPath{ mut widget, path }) = active_widgets.keyboard.and_then(|kbd| widget_stack.move_to_widget(kbd)) {
        ...
        where R: Renderer<Frame=F>
    {
        let Root {
            ...
            ref mut timer_list,
            ref mut widget_state_map,
            ref mut update_receiver,
            ref mut widget_stack_base,
            ..
        } = *self;

        while let Some(attachment) = update_receiver.recv_attachment() {
            match attachment {
                BusEvent::Attach(widget_id) => {widget_state_map.entry(widget_id).or_default();},
                BusEvent::Detach(widget_id) => {widget_state_map.remove(&widget_id);}
            }
        }

        let mut widget_stack = widget_stack_base.use_stack_dyn(&mut *root_widget, root_id);

        let mut redraw = false;
        while let Some((event, source)) = update_receiver.recv_item() {
            match event {
                // TODO: Examine possibility of only redrawing specific widgets
                UpdateEvent::Render => redraw = true,
                UpdateEvent::Child => (), // TODO: CHECK INTEGRITY OF CHILD TREE
                UpdateEvent::Layout => widget_stack.move_over_widgets(Some(source).into_iter(), |mut widget, _, _| {
                    if let Some(mut parent) = widget.as_parent_mut() {
                        parent.update_child_layout();
                        redraw = true;
                    }
                }),
                UpdateEvent::Timer => widget_stack.move_over_widgets(Some(source).into_iter(), |widget, _, _| {
